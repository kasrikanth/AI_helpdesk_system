...
import {sendMessage, formatChatResponse}from '../backend_connection/chat';
...
useEffect(() => {
  if (!hasRestored) return; // 🚨 DO NOT SAVE BEFORE RESTORE FINISHES
...
// Initialize with welcome message (only after restore)
useEffect(() => {
...
}, [isOpen, collapsed, messages.length, hasRestored]);
  ...
  const handleSendMessage = useCallback(async (userMessage) => {
    ...
    setMessages(prev => [...prev, userMsg]);
      ...
      const formatted = formatChatResponse(result.data);
      ...
      await new Promise(resolve =>
        setTimeout(resolve, Math.min(formatted.message.length * 20, 2000))
      );
      ...
      // ===============================
      // HANDLE ESCALATION FROM BACKEND
      // ===============================
      ...
      if (formatted.needsEscalation && formatted.ticketId) {
        ...
        setConversationContext(prev => ({
          ...prev,
          ticketCreated: true,
          lastTicketId: formatted.ticketId,
        }));

        if (onTicketCreated) {
          onTicketCreated({
            id: formatted.ticketId,
            tier: formatted.tier,
  ...
  const handleClose = () => {
    ...
    // localStorage.removeItem("ai_chat_messages");       ------>>> if you want to restart conv by closing chatwidget uncommant this line 
    setMessages([]); 
