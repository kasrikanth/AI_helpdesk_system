    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', width: '100vw', overflow: 'hidden' }}>
      ...
      <AppBar position="static" sx={{ backgroundColor: '#121212', borderBottom: '1px solid #333333', width: '100%' }}>
        <Toolbar sx={{ py: 1, minHeight: '64px !important', width: '100%', px: 3 }}>
          <Button
            // component={RouterLink}             //  uncomment to make header to work ashome tab
            // to="/"
          ...
          {user && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0 }}>
              ...
              {/* Tickets - for support_engineer and admin */}
              ...
              {/* Analytics - for instructor, support_engineer, and admin */}
              {(user.role === 'instructor' || user.role === 'support_engineer' || user.role === 'admin') && (
                <>
                  <Divider orientation="vertical" flexItem sx={{ mx: 0.5, borderColor: 'rgba(255, 255, 255, 0.2)', height: '24px', alignSelf: 'center' }} />
                  <Button
                    component={RouterLink}
              ...
              {/* Escalation - for instructor, support_engineer, and admin */}
              {(user.role === 'instructor' || user.role === 'support_engineer' || user.role === 'admin') && (
                <>
                  <Divider orientation="vertical" flexItem sx={{ mx: 0.5, borderColor: 'rgba(255, 255, 255, 0.2)', height: '24px', alignSelf: 'center' }} />
                  <Button
              ...
              {/* Integration - for support_engineer and admin */}
              {(user.role === 'support_engineer' || user.role === 'admin') && (
                <>
                  <Divider orientation="vertical" flexItem sx={{ mx: 0.5, borderColor: 'rgba(255, 255, 255, 0.2)', height: '24px', alignSelf: 'center' }} />
                  <Button
              ...
              {/* Model Versioning - for support_engineer and admin */}
              {(user.role === 'support_engineer' || user.role === 'admin') && (
                <>
                  <Divider orientation="vertical" flexItem sx={{ mx: 0.5, borderColor: 'rgba(255, 255, 255, 0.2)', height: '24px', alignSelf: 'center' }} />
                  <Button
      ...
      <Box sx={{ backgroundColor: '#121212', borderTop: '1px solid #333333', py: 2, mt: 4, width: '100%', flexShrink: 0 }}>
        <Box sx={{ px: 3 }}>
          <Typography variant="caption" sx={{ color: '#999999' }}>
            © 2026 EASYSTEPIN PVT limted. ESI AI HelpDesk.
          </Typography>
        </Box>
      </Box>
    </Box>
